...
export async function gethitokoto() {
  const response = await axios.get(
    "https://international.v1.hitokoto.cn?c=a&c=b&c=c&c=d&c=h&c=i&c=j&c=k"
  );
  if (response.status !== 200) {
    throw new Error(response.statusText);
  }
...
export async function getxlog(subject_id) {
  return await axiosInstance.get("/proxy_xlog");
}
...
export async function getMessages(limit = 10, order = "desc", page = 1) {
  limit = Number(limit) || 10;
  page = Number(page) || 1;
  return fetchData(`/get_messages?limit=${limit}&order=${order}&page=${page}`);
}
