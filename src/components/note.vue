<template>
    <div class="start-page">
        <div v-for="item in result.list" :key="item.id" class="onepart"  style="cursor: pointer;transition: background-color .18s; border-radius: 6px;"
                @mouseover="$event.currentTarget.style.backgroundColor = 'rgb(41 149 126 /20%)'"
                @mouseleave="$event.currentTarget.style.backgroundColor = 'transparent'">
                <router-link
                    :to="{ name: 'page', params: { id: item.noteId } }"
                    style="text-decoration: none; color: inherit; display: contents;"
                >
            <h2 style="margin: 5px; display: block; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; transition: background-color .18s, color .18s;">
            {{ item.metadata.content.title }}
            </h2>
            <div style="width:100%; padding-top:56.25%; position:relative; border-radius:6px; overflow:hidden;">
            <div v-if="item.metadata.content?.cover"
                 :style="{
                     position: 'absolute',
                     top: 0,
                     left: 0,
            ...
            <p v-html="md.render(item.metadata.content.summary)"
                style="word-wrap:break-word;font-size:small;display:-webkit-box;line-clamp:3;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden;text-overflow:ellipsis;"></p>
            </router-link>
        </div>
    </div>
</template>
